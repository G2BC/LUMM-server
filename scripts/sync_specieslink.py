...
def _fetch_page(scientific_name, offset):
    ...
    for attempt in range(MAX_RETRIES):
        ...
        if r.status_code == 429:
            wait = 2**attempt * 10
            print(
                f"  429 recebido — aguardando {wait}s (tentativa {attempt + 1}/{MAX_RETRIES})",
                flush=True,
            )
            time.sleep(wait)
            continue
...
class SpeciesLinkSync(SyncRunner):
    source_name = "specieslink"
    env_prefix = "SPECIESLINK"

    def get_species_rows(self, session, lumm_ids):
        query = session.query(Species.id, Species.scientific_name, Species.group_name).filter(
    ...
    def sync_species(self, row, start_time):
        ...
        while True:
            ...
            data = _fetch_page(row.scientific_name, offset)
            ...
            for feature in features:
                ...
                if collection_id and catalog_number:
                    url = (
                        f"https://specieslink.net/search/records/collectioncode/"
                        f"{collection_id}/catalognumber/{catalog_number}"
                    )
                else:
                    url = None

                rows_by_external_id[ext_id] = {
                    "species_id": row.id,
                    "source": "specieslink",
                    "external_id": ext_id,
                    "latitude": float(lat),
            ...
            page += 1
            self._log(
                f"  [{row.id}] página {page}: {len(features)} features, "
        ...
        CacheService.delete(f"{prefix}:{row.id}:specieslink")

        return total, deleted, True


if __name__ == "__main__":
    SpeciesLinkSync(create_app()).run()
