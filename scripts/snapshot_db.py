...
def _build_row_pt(s) -> dict:
    ...
    return {
        ...
        "Taxonomia": {
            ...
            "Ano de publicação efetiva": t.years_of_effective_publication if t else None,
        ...
        "Características": {
            "Anatomia bioluminescente": {
                ...
                "Estipe": c.lum_stipe if c else None,
                "Píleo": c.lum_pileus if c else None,
                "Lamelas": c.lum_lamellae if c else None,
                "Esporos": c.lum_spores if c else None,
            },
            "Habitats": [h.label_pt for h in c.habitats] if c else [],
            "Formas de crescimento": [g.label_pt for g in c.growth_forms] if c else [],
            "Substratos": [sub.label_pt for sub in c.substrates] if c else [],
            "Modos de nutrição": [n.label_pt for n in c.nutrition_modes] if c else [],
            ...
            "Dicas de busca": c.finding_tips_pt if c else None,
            "Árvores próximas": c.nearby_trees_pt if c else None,
            ...
            "Tamanho médio (cm)": c.size_cm if c else None,
            "Sazonalidade (Início)": c.season_start_month if c else None,
            "Sazonalidade (Fim)": c.season_end_month if c else None,
            "Distribuição": [f"({d.slug}) {d.label_pt}" for d in s.distributions],
...
def _build_row_en(s) -> dict:
    ...
    return {
        ...
        "Taxonomy": {
            ...
            "Year of effective publication": t.years_of_effective_publication if t else None,
        ...
        "Characteristics": {
            "Bioluminescent anatomy": {
                ...
                "Stipe": c.lum_stipe if c else None,
                "Pileus": c.lum_pileus if c else None,
                "Lamellae": c.lum_lamellae if c else None,
                "Spores": c.lum_spores if c else None,
            },
            "Habitats": [h.label_en for h in c.habitats] if c else [],
            "Growth forms": [g.label_en for g in c.growth_forms] if c else [],
            "Substrates": [sub.label_en for sub in c.substrates] if c else [],
            "Nutrition modes": [n.label_en for n in c.nutrition_modes] if c else [],
            ...
            "Nearby trees": c.nearby_trees if c else None,
            ...
            "Average size (cm)": c.size_cm if c else None,
            "Season start": c.season_start_month if c else None,
            "Season end": c.season_end_month if c else None,
            "Distribution": [f"({d.slug}) {d.label_en}" for d in s.distributions],
